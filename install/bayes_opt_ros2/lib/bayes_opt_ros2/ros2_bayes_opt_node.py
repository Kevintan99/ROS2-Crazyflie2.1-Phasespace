...
class BayesOptNode(Node):
    def __init__(self) -> None:
        ...
        # Continuous publishing parameters
        ...
        # Continuous publishing settings
        self.continuous_publish = bool(self.get_parameter("continuous_publish").value)
        ...
        # Continuous publishing state
        self._current_params: List[float] | None = None
        self._should_publish = False
        self._publish_timer = None
        
        # Setup continuous publishing timer if enabled
        if self.continuous_publish:
            timer_period = 1.0 / self.publish_rate_hz
            self._publish_timer = self.create_timer(timer_period, self._continuous_publish_callback)
            self.get_logger().info(
                f"Continuous publishing enabled at {self.publish_rate_hz} Hz"
            )
    ...
    def _continuous_publish_callback(self) -> None:
        """Timer callback for continuous publishing of current parameters."""
        with self._lock:
            if self._should_publish and self._current_params is not None:
                self._publish_params_internal(self._current_params, log_msg=False)
    
    def _start_continuous_publishing(self, params: List[float]) -> None:
        """Start continuously publishing the given parameters."""
        with self._lock:
            self._current_params = params.copy()
            self._should_publish = True
            
    def _stop_continuous_publishing(self) -> None:
        """Stop continuously publishing parameters."""
        with self._lock:
            self._should_publish = False
    ...
    def destroy_node(self) -> bool:
        ...
        self._result_event.set()
        self._allow_event.set()
        self._stop_continuous_publishing()
        if self._publish_timer:
            self._publish_timer.cancel()
        return super().destroy_node()

    # ---------------------- Objective + orchestration ----------------------
    def _publish_params_internal(self, vec: List[float], log_msg: bool = True) -> None:
        """Internal method to publish parameters."""
    ...
    def _publish_params(self, vec: List[float]) -> None:
        """Public method to publish parameters (always logs)."""
        self._publish_params_internal(vec, log_msg=True)
    ...
    def _objective(self, **kwargs: float) -> float:
        ...
        # Start continuous publishing if enabled
        if self.continuous_publish:
            self._start_continuous_publishing(vec)
        ...
        # Stop continuous publishing once we have results
        self._stop_continuous_publishing()
